...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        self.user = user_model.objects.create_staffuser(email=email, password=password, first_name=first_name, last_name=last_name)
        log_in = self.client.login(email=email, password=password)

        # confirm user is logged in
        self.assertTrue(log_in)
        self.assertTrue(self.user.is_staff)
        # create a blog post for the test db
        title = 'test title'
        category = BlogCategory.objects.create(name='testCategory')
        content = 'testContent'
        self.blog = BlogPost.objects.create(title=title, category=category, author=self.user, content=content)
    ...
    def test_blog_post_detail_template(self):
        ...
        post = posts[0]
        response = self.client.get(f'/blog/{post.slug}/')
        self.assertTemplateUsed(response, template_name='blog/blog_post_detail.html')
    ...
    def test_add_blog_post_page_template(self):
        ...
        '''
        response = self.client.get('/blog/add-post/')
        self.assertTemplateUsed(response, template_name='blog/blog_post_form.html')
    ...
    def test_adding_blog_post(self):
        ...
        data = urlencode({'title': 'page submitted blog post', 'category': '1', 'image_url': 'https://en.wikipedia.org/wiki/File:Test_image.jpg', 'image': None, 'content': 'page submitted blog post content'})
        response = self.client.post('/blog/add-post/', data, content_type='application/x-www-form-urlencoded')
    ...
    def test_editing_blog_post(self):
        ...
        slug = post.slug
        response = self.client.post(reverse('edit_blog_post', kwargs={'slug': slug}), {'title': 'test edited title', 'category': '1', 'content': 'test content'})
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_blog_post(self):
        ...
        post = BlogPost.objects.get(title='test title')
        self.assertEqual(BlogPost.objects.count(), 1)
        response = self.client.post(reverse('delete_blog_post', kwargs={'pk': post.id}))
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0].tags, 'success')
        self.assertEqual(response.status_code, 302)
    ...
    def test_logged_out_user_delete_blog_post(self):
        ...
        self.assertEqual(BlogPost.objects.count(), 1)
        self.client.logout()
        response = self.client.get(reverse('delete_blog_post', kwargs={'pk': post.id}))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(BlogPost.objects.count(), 1)
